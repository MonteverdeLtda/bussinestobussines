...
var PagesEmployeesSingleContactsAdd = Vue.extend({
	template: '#page-employees-single-contacts-add',
	data: function() {
		return {
		...
		load_scripts(){
			...
			self.jvalidate = $("#jvalidate").validate({
				...
				submitHandler: function() {
					...
					FG.api('POST', '/employees_contacts', self.post, function (b) {
						if(Number(b) > 0)
						{
							$.notify("El contacto fue añadido correctamente.!", "success");
							router.push({
								name: 'page-employees-single-contacts-list',
								params: {
