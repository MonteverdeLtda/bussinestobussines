var PagesSystemTypesIdentifications = Vue.extend({
	template: '#page-system-types-identifications',
	data: function() {
		return {
			list: [],
			create: {
	...
	methods: {
		...
		find(){
			...
			FG.api('GET', '/types_identifications', {
			}, function(a){
		...
		delete_element(idDelete){
			...
			bootbox.confirm({
				...
				callback: function (a) {
					if(a === true){
						FG.api('DELETE','/types_identifications/' + idDelete, {
						}, function(r){
							if(r == true)
							{
		...
		create_element(){
			...
			bootbox.confirm({
				...
				callback: function (a) {
					if(a === true){
						FG.api('POST','/types_identifications', self.create, function(r){
							if(Number(r) > 0)
							{
		...
		edit_element(){
			...
			bootbox.confirm({
				...
				callback: function (a) {
					if(a === true){
						FG.api('PUT','/types_identifications/' + self.edit.id, self.edit, function(r){
							if(Number(r) > 0)
							{
