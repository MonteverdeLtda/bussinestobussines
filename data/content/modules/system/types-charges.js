var PagesSystemTypesCharges = Vue.extend({
	template: '#page-system-types-charges',
	data: function() {
		return {
			list: [],
			create: {
	...
	methods: {
		...
		delete_element(idDelete){
			...
			bootbox.confirm({
				...
				callback: function (a) {
					if(a === true){
						FG.api('DELETE','/types_charges/' + idDelete, {
						}, function(r){
							if(r == true)
							{
		...
		create_element(){
			...
			bootbox.confirm({
				...
				callback: function (a) {
					if(a === true){
						FG.api('POST','/types_charges', self.create, function(r){
							if(Number(r) > 0)
							{
		...
		edit_element(){
			...
			bootbox.confirm({
				...
				callback: function (a) {
					if(a === true){
						FG.api('PUT','/types_charges/' + self.edit.id, self.edit, function(r){
							if(Number(r) > 0)
							{
