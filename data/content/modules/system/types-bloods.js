var PagesSystemTypesBloods = Vue.extend({
	template: '#page-system-types-bloods',
	data: function() {
		return {
			list: [],
			create: {
	...
	methods: {
		...
		delete_element(idDelete){
			...
			bootbox.confirm({
				...
				callback: function (a) {
					if(a === true){
						FG.api('DELETE','/types_bloods/' + idDelete, {
						}, function(r){
							if(r == true)
							{
		...
		create_element(){
			...
			bootbox.confirm({
				...
				callback: function (a) {
					if(a === true){
						FG.api('POST','/types_bloods', self.create, function(r){
							if(Number(r) > 0)
							{
		...
		edit_element(){
			...
			bootbox.confirm({
				...
				callback: function (a) {
					if(a === true){
						FG.api('PUT','/types_bloods/' + self.edit.id, self.edit, function(r){
							if(Number(r) > 0)
							{
