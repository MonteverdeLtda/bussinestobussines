var PagesSystemTypesLettersAddresses = Vue.extend({
	template: '#page-system-types-letters-addresses',
	data: function() {
		return {
			list: [],
			create: {
	...
	methods: {
		...
		find(){
			var self = this;
			FG.api('GET', '/types_letters_addresses', {
			}, function(a){
		...
		delete_element(idDelete){
			...
			bootbox.confirm({
				...
				callback: function (a) {
					if(a === true){
						FG.api('DELETE','/types_letters_addresses/' + idDelete, {
						}, function(r){
							if(r == true)
							{
		...
		create_element(){
			...
			bootbox.confirm({
				...
				callback: function (a) {
					if(a === true){
						FG.api('POST','/types_letters_addresses', self.create, function(r){
							if(Number(r) > 0)
							{
		...
		edit_element(){
			...
			bootbox.confirm({
				...
				callback: function (a) {
					if(a === true){
						FG.api('PUT','/types_letters_addresses/' + self.edit.id, self.edit, function(r){
							if(Number(r) > 0)
							{
