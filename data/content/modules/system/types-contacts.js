var PagesSystemTypesContacts = Vue.extend({
	template: '#page-system-types-contacts',
	data: function() {
		return {
			list: [],
			create: {
	...
	methods: {
		...
		delete_element(idDelete){
			...
			bootbox.confirm({
				...
				callback: function (a) {
					if(a === true){
						FG.api('DELETE','/types_contacts/' + idDelete, {
						}, function(r){
							if(r == true)
							{
		...
		create_element(){
			...
			bootbox.confirm({
				...
				callback: function (a) {
					if(a === true){
						FG.api('POST','/types_contacts', self.create, function(r){
							if(Number(r) > 0)
							{
		...
		edit_element(){
			...
			bootbox.confirm({
				...
				callback: function (a) {
					if(a === true){
						FG.api('PUT','/types_contacts/' + self.edit.id, self.edit, function(r){
							if(Number(r) > 0)
							{
