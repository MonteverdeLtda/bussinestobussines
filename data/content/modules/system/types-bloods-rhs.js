var PagesSystemTypesBloodsRHs = Vue.extend({
	template: '#page-system-types-bloods-rhs',
	data: function() {
		return {
			list: [],
			create: {
	...
	methods: {
		...
		delete_element(idDelete){
			...
			bootbox.confirm({
				...
				callback: function (a) {
					if(a === true){
						FG.api('DELETE','/types_bloods_rhs/' + idDelete, {
						}, function(r){
							if(r == true)
							{
		...
		create_element(){
			...
			bootbox.confirm({
				...
				callback: function (a) {
					if(a === true){
						FG.api('POST','/types_bloods_rhs', self.create, function(r){
							if(Number(r) > 0)
							{
		...
		edit_element(){
			...
			bootbox.confirm({
				...
				callback: function (a) {
					if(a === true){
						FG.api('PUT','/types_bloods_rhs/' + self.edit.id, self.edit, function(r){
							if(Number(r) > 0)
							{
