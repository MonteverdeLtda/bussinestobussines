var PagesAccountsEdit = Vue.extend({
	...
	methods: {
		...
		find(){
			...
			FG.api('GET', '/accounts/' + self.$route.params.account_id, {
				...
				if(r != undefined && r.id > 0){
					...
					if(r.address_principal != null && Number(r.address_principal)){
						FG.api('GET', '/addresses/' + r.address_principal, {
						}, function(s){
							if(s != undefined && s.id > 0){ self.address_principal = s; }
							
						});
					};
					
					if(r.address_invoices != null && Number(r.address_invoices)){
						FG.api('GET', '/addresses/' + r.address_invoices, {
						}, function(s){
							if(s != undefined && s.id > 0){ self.address_invoices = s; }
		...
		delete_this(){
			...
			console.log(self.$route.params.account_id);
			
			bootbox.confirm({
				title: "Estas Seguro?",
				message: "¿Quieres eliminar la cuenta? Esto no se puede deshacer.",
				buttons: {
					cancel: {
						label: '<i class="fa fa-times"></i> Cancelar'
					},
				...
				callback: function (result) {
					if(result == true){
						...
						FG.api('GET', '/accounts/' + self.$route.params.account_id, {
							...
							if(r != undefined && r.id > 0){
								FG.api('DELETE', '/accounts/' + self.$route.params.account_id, {}, function(s){
									if(Number(s) > 0){
										$.notify("Cuenta eliminada con éxito.", 'success');
										router.push({ name: 'page-accounts-list', params: {} });
									}else{
